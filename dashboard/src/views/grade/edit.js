...
export default function EditCourse() {
    ...
    return (
        <>
            ...
            {
                getGrade.isLoading || getCourses.isLoading ? 
                    <PageLoading />
                : <>
                    {
                                        ...
                                        <div className="col-12 col-md-8 offset-md-2">
                                            <form onSubmit={handleSubmit(submitHandler)}>
                                                ...
                                                <select className={`form-control form-control-lg mb-3 ${errors.currentCourseId ? 'border-danger' : ''}`} aria-label="select currentCourse"
                                                    defaultValue={getGrade.data.grade.currentCourse.id || "" }
                                                    {...register("currentCourseId", { required: true })}
                                                >
                                                    <option value="">select grade course</option>
                                                    {getCourses.data.courses.map(course => <option value={course.id} key={course.id} >{course.name}</option>)}

                                                </select>
                                                {
                                                    updateGradeResult.error?.data?.error?.errors?.currentCourseId ?
                                                        <p className="alert alert-danger">{updateGradeResult.error.data.error.errors.currentCourseId}</p> : null
                                                }
